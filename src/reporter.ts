import type { ApiCheckResult, QaAnalysis } from './types.js';

interface RenderReportInput {
  analysis: QaAnalysis;
  liveResults: ApiCheckResult[];
}

export function renderReport({ analysis, liveResults }: RenderReportInput): string {
  const lines: string[] = [];
